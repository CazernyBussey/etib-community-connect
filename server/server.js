...
app.post("/api/auth/login", authLimiter, async (req, res) => {
  try {
    ...
    const refreshed = await get(
      "SELECT id, full_name, email, role, status FROM users WHERE lower(email)=lower(?)",
      [emailNorm]
    );
...
app.get("/api/listings", async (req, res) => {
  ...
  if (listingType) {
    if (listingType === "Both") where += " AND l.listing_type='Both'";
    else { where += " AND (l.listing_type=? OR l.listing_type='Both')"; params.push(listingType); }
  }
  if (q) {
    where += ` AND (lower(l.business_name) LIKE ? OR lower(l.short_summary) LIKE ? OR lower(l.full_description) LIKE ? OR lower(l.category) LIKE ?)`;
    const like = `%${q}%`;
    params.push(like, like, like, like);
  }
  if (location) { where += " AND lower(l.city || ' ' || l.state || ' ' || l.service_area_type) LIKE ?"; params.push(`%${location}%`); }
...
app.get("/api/listings/:id", async (req, res) => {
  ...
  const row = await get(`SELECT id, business_name, owner_contact_name, business_email, phone, text_number, website_url, listing_type, category, short_summary, full_description, listen_summary, supports_bvi, accessibility_details, primary_contact_method, city, state, service_area_type, hours, languages, remote_details, inperson_notes, social_links, certifications, testimonial, status, last_updated, is_featured, featured_rank FROM listings WHERE id=? AND status='approved'`, [id]);
  if (!row) return res.status(404).json({ error: "Listing not found" });
  const ratingSummary = await get(`SELECT COUNT(*) AS review_count, ROUND(AVG(rating), 1) AS average_rating FROM reviews WHERE listing_id=? AND status='approved'`, [id]);
...
app.get("/api/listings/:id/reviews", async (req, res) => {
  const listingId = Number(req.params.id);
  if (!Number.isInteger(listingId) || listingId <= 0) return res.status(400).json({ error: "Invalid listing id" });
  const reviews = await all(`SELECT id, reviewer_name, rating, review_text, created_at FROM reviews WHERE listing_id=? AND status='approved' ORDER BY datetime(approved_at) DESC, datetime(created_at) DESC LIMIT 100`, [listingId]);
  const summary = await get(`SELECT COUNT(*) AS review_count, ROUND(AVG(rating), 1) AS average_rating FROM reviews WHERE listing_id=? AND status='approved'`, [listingId]);
  res.json({ reviews, summary: { reviewCount: summary?.review_count || 0, averageRating: summary?.average_rating || null } });
});
...
app.get("/api/admin/listings", authRequired, adminRequired, async (req, res) => {
  ...
  if (q) { where += " AND (lower(l.business_name) LIKE ? OR lower(l.owner_contact_name) LIKE ? OR lower(l.business_email) LIKE ?)"; const like = `%${q}%`; params.push(like, like, like); }
  const rows = await all(`SELECT l.id, l.business_name, l.owner_contact_name, l.business_email, l.phone, l.category, l.listing_type, l.status, l.admin_note, l.last_updated, l.created_at, l.short_summary, l.full_description, l.supports_bvi, l.accessibility_details, l.city, l.state, l.website_url, l.is_featured, l.featured_rank, COALESCE(ROUND((SELECT AVG(r.rating) FROM reviews r WHERE r.listing_id = l.id AND r.status='approved'), 1), NULL) AS average_rating, COALESCE((SELECT COUNT(*) FROM reviews r WHERE r.listing_id = l.id AND r.status='approved'), 0) AS review_count, u.full_name AS owner_name, u.email AS owner_email, u.phone AS owner_phone, u.status AS owner_status FROM listings l LEFT JOIN users u ON u.id = l.owner_user_id ${where} ORDER BY CASE WHEN l.status='pending' THEN 0 ELSE 1 END, datetime(l.created_at) DESC LIMIT 200`, params);
  res.json({ listings: rows });
});
...
app.patch("/api/admin/listings/:id", authRequired, adminRequired, async (req, res) => {
  ...
  await logAdminAction({ adminUserId: req.user.sub, action: `listing_status_${String(status)}`, targetType: "listing", targetId: id, meta: { adminNote: adminNote ? String(adminNote).trim() : null } });
  ...
  await logAdminAction({ adminUserId: req.user.sub, action: emailSent ? "listing_notification_sent" : "listing_notification_failed", targetType: "listing", targetId: id, meta: { status, to } });
...
app.patch("/api/admin/listings/:id/feature", authRequired, adminRequired, async (req, res) => {
  ...
  const listing = await get(`SELECT id, business_name, status, is_featured, featured_rank FROM listings WHERE id=?`, [id]);
  if (!listing) return res.status(404).json({ error: "Listing not found" });
  if (listing.status !== "approved") return res.status(400).json({ error: "Only approved listings can be featured" });
  const wantFeatured = Number(isFeatured) === 1;
  if (!wantFeatured) {
    await run(`UPDATE listings SET is_featured=0, featured_rank=NULL, last_updated=datetime('now') WHERE id=?`, [id]);
    await logAdminAction({ adminUserId: req.user.sub, action: "listing_feature_removed", targetType: "listing", targetId: id, meta: { businessName: listing.business_name } });
    return res.json({ ok: true });
  }
  const rank = Number(featuredRank);
  if (!Number.isInteger(rank) || rank < 1 || rank > 5) return res.status(400).json({ error: "featuredRank must be 1 through 5" });
  await run("BEGIN TRANSACTION");
  try {
    await run(`UPDATE listings SET is_featured=0, featured_rank=NULL, last_updated=datetime('now') WHERE featured_rank=? AND is_featured=1 AND id<>?`, [rank, id]);
    await run(`UPDATE listings SET is_featured=1, featured_rank=?, last_updated=datetime('now') WHERE id=?`, [rank, id]);
    await run("COMMIT");
  } catch {
    await run("ROLLBACK").catch(() => {});
    return res.status(500).json({ error: "Could not update featured placement" });
  }
  await logAdminAction({ adminUserId: req.user.sub, action: "listing_feature_set", targetType: "listing", targetId: id, meta: { businessName: listing.business_name, featuredRank: rank } });
  return res.json({ ok: true });
});
...
app.get("/api/admin/users", authRequired, adminRequired, async (req, res) => {
  ...
  let where = "WHERE 1=1";
  const params = [];
  if (!includeHidden) where += " AND COALESCE(u.is_hidden, 0)=0";
  if (status) { where += " AND u.status=?"; params.push(status); }
  if (q) { where += " AND (lower(u.full_name) LIKE ? OR lower(u.email) LIKE ? OR lower(u.phone) LIKE ?)"; const like = `%${q}%`; params.push(like, like, like); }
  const rows = await all(`SELECT u.id, u.full_name, u.email, u.phone, u.role, u.status, u.created_at, u.approved_at, COALESCE(u.is_hidden, 0) AS is_hidden, approver.full_name AS approved_by_name, COALESCE((SELECT COUNT(*) FROM listings l WHERE l.owner_user_id = u.id), 0) AS listing_count, COALESCE((SELECT COUNT(*) FROM listings l WHERE l.owner_user_id = u.id AND l.status='pending'), 0) AS pending_listing_count, COALESCE((SELECT COUNT(*) FROM listings l WHERE l.owner_user_id = u.id AND l.status='approved'), 0) AS approved_listing_count FROM users u LEFT JOIN users approver ON approver.id = u.approved_by_user_id ${where} ORDER BY CASE WHEN u.status='pending' THEN 0 WHEN u.status='approved' THEN 1 ELSE 2 END, datetime(u.created_at) DESC LIMIT 1000`, params);
  const summary = await get(`SELECT COALESCE(SUM(CASE WHEN status='pending' AND COALESCE(is_hidden,0)=0 THEN 1 ELSE 0 END), 0) AS pending_users, COALESCE(SUM(CASE WHEN status='approved' AND COALESCE(is_hidden,0)=0 THEN 1 ELSE 0 END), 0) AS approved_users, COALESCE(SUM(CASE WHEN status='rejected' AND COALESCE(is_hidden,0)=0 THEN 1 ELSE 0 END), 0) AS rejected_users, COALESCE(SUM(CASE WHEN COALESCE(is_hidden,0)=1 THEN 1 ELSE 0 END), 0) AS hidden_users FROM users`);
...
app.patch("/api/admin/users/:id/status", authRequired, adminRequired, async (req, res) => {
  ...
  if (!valid.includes(String(status))) return res.status(400).json({ error: "Invalid user status" });
  const user = await get("SELECT id, full_name, email, role, status, COALESCE(is_hidden, 0) AS is_hidden FROM users WHERE id=?", [id]);
  ...
  await logAdminAction({ adminUserId: req.user.sub, action: `user_status_${String(status)}`, targetType: "user", targetId: id, meta: { email: user.email, emailSent, hideUser } });
...
app.get("/api/admin/reviews", authRequired, adminRequired, async (req, res) => {
  ...
  if (q) { where += " AND (lower(l.business_name) LIKE ? OR lower(r.reviewer_name) LIKE ? OR lower(r.review_text) LIKE ?)"; const like = `%${q}%`; params.push(like, like, like); }
  const rows = await all(`SELECT r.id, r.listing_id, r.reviewer_name, r.reviewer_email, r.rating, r.review_text, r.status, r.admin_note, r.created_at, r.approved_at, l.business_name FROM reviews r LEFT JOIN listings l ON l.id = r.listing_id ${where} ORDER BY datetime(r.created_at) DESC LIMIT 300`, params);
  res.json({ reviews: rows });
});

app.patch("/api/admin/reviews/:id", authRequired, adminRequired, async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ error: "Invalid review id" });
  const { status, adminNote } = req.body || {};
  const valid = ["pending", "approved", "rejected"];
  if (!valid.includes(String(status))) return res.status(400).json({ error: "Invalid status" });
  ...
  await logAdminAction({ adminUserId: req.user.sub, action: `review_status_${String(status)}`, targetType: "review", targetId: id, meta: { adminNote: adminNote ? String(adminNote).trim() : null } });
...
app.get("/api/admin/reports", authRequired, adminRequired, async (req, res) => {
  const rows = await all(`SELECT r.id, r.listing_id, r.reporter_email, r.reason, r.created_at, l.business_name FROM reports r LEFT JOIN listings l ON l.id=r.listing_id ORDER BY datetime(r.created_at) DESC LIMIT 200`);
  res.json({ reports: rows });
});

app.get("/api/admin/audit-logs", authRequired, adminRequired, async (req, res) => {
  const rows = await all(`SELECT a.id, a.admin_user_id, a.action, a.target_type, a.target_id, a.meta_json, a.created_at, u.full_name AS admin_name, u.email AS admin_email FROM admin_audit_logs a LEFT JOIN users u ON u.id = a.admin_user_id ORDER BY datetime(a.created_at) DESC LIMIT 500`);
  res.json({ logs: rows });
});
